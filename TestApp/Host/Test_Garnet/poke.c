...
{
	fprintf (fout, "Usage: %s  <addr>  <data>    (can use decimal or 0xHEX format)\n",
		 argv [0]);
}
...
int main (int argc, char **argv)
{
	...
	uint32_t wdata = strtoul (argv[2], 0, 0);
	...
	*((uint32_t *) virt_addr) = wdata;
	fprintf (stdout, "Poke address 0x%08x <= 0x%08x\n", addr, wdata);

...
void buf_write_AXI4 (uint8_t *wbuf, const int n_bytes, const uint64_t addr)
{
	...
	fprintf (stdout, "%s: %0d bytes to addr 0x%0" PRIx64 "\n",
		 __FUNCTION__, n_bytes, addr);

    rc = AWSteria_AXI4_write (AWSteria_Host_state, wbuf, n_bytes, addr);
    if (rc != 0) {
	if (verbosity_AXI4_W == 0)
	    fprintf (stdout, "ERROR: %s: %0d bytes to addr 0x%0" PRIx64 "\n",
		     __FUNCTION__, n_bytes, addr);
		    ...
		    const uint8_t *wbuf)
{
	...
	fprintf (stdout, "%s: %0d bytes from addr 0x%0" PRIx64 "\n",
		 __FUNCTION__, n_bytes, addr);

    rc = AWSteria_AXI4_read (AWSteria_Host_state, rbuf, n_bytes, addr);
    if (rc != 0) {
	if (verbosity_AXI4_R == 0)
	    fprintf (stdout, "ERROR: %s: %0d bytes from addr 0x%0" PRIx64 "\n",
		     __FUNCTION__, n_bytes, addr);
	fprintf (stdout, "    FAILED: rc = %0d\n", rc);
	num_ERRORS++;
	return;
...
void buf_write_AXI4L (uint32_t *wbuf, const uint64_t addr)
{
	...
	fprintf (stdout, "ERROR: %s: addr %0" PRIx64 " is > 32 bits\n", __FUNCTION__, addr);
	return;
    }
    ...
    if (rc != 0) {
	fprintf (stdout, "ERROR: %s: 4 bytes to addr 0x%0" PRIx64 "\n", __FUNCTION__, addr);
	fprintf (stdout, "    FAILED: rc = %0d\n", rc);
	num_ERRORS++;
    }
    else {
...
void test0 (uint64_t base_addr)
{
    fprintf (stdout, "\n");
    fprintf (stdout, "%s: ----------------\n", __FUNCTION__);
    fprintf (stdout, "AXI4: Single-byte write, base_addr 0x%0" PRIx64 "\n", base_addr);
    buf_write_AXI4 (wbuf, 1, base_addr);
    fprintf (stdout, "AXI4: Single-byte read, base_addr 0x%0" PRIx64 "\n", base_addr);
...
void test50 (uint64_t base_addr)
{
    ...
    fprintf (stdout, "    base_addr = 0x%0" PRIx64 "\n", base_addr);
...
void test60 (int size, uint64_t base_addr)
{
    fprintf (stdout, "\n");
    fprintf (stdout, "%s: ----------------\n", __FUNCTION__);
    fprintf (stdout, "AXI4: write/read 0x%0x bytes, base_addr 0x%0" PRIx64 "\n", size, base_addr);
...
void test70 ()
{
    ...
    if (test_DDR_A) {
	fprintf (stdout,
		 "AXI4: write DDR_A (addr 0x%0" PRIx64 "); data 0x%0" PRIx64 " to 0x%0" PRIx64 "\n",
		 ddr_A_base, offset_A, offset_A + size);
	buf_write_AXI4 (& (wbuf [offset_A]), size, ddr_A_base);
    }
    if (test_DDR_B) {
	fprintf (stdout,
		 "AXI4: write DDR_B (addr 0x%0" PRIx64 "); data 0x%0" PRIx64 " to 0x%0" PRIx64 "\n",
		 ddr_B_base, offset_B, offset_B + size);
	buf_write_AXI4 (& (wbuf [offset_B]), size, ddr_B_base);
    }
    if (test_DDR_C) {
	fprintf (stdout,
		 "AXI4: write DDR_C (addr 0x%0" PRIx64 "); data 0x%0" PRIx64 " to 0x%0" PRIx64 "\n",
		 ddr_C_base, offset_C, offset_C + size);
	buf_write_AXI4 (& (wbuf [offset_C]), size, ddr_C_base);
    }
    if (test_DDR_D) {
	fprintf (stdout,
		 "AXI4: write DDR_D (addr 0x%0" PRIx64 "); data 0x%0" PRIx64 " to 0x%0" PRIx64 "\n",
...
void test990 ()
{
    ...
    fprintf (stdout, "AXI4: Single-byte write, out-of-bounds addr 0x%0" PRIx64 "\n",
	     out_of_bounds_addr);
    fprintf (stdout, "NOTE: ERROR expected\n");
    buf_write_AXI4 (wbuf, 1, out_of_bounds_addr);

    fprintf (stdout, "----\n");
    fprintf (stdout, "AXI4: Single-byte read, out-of-bounds addr 0x%0" PRIx64 "\n",
	     out_of_bounds_addr);
    fprintf (stdout, "NOTE: ERROR expected\n");
...
int main (int argc, char *argv [])
{
    ...
    fprintf (stdout, "    ddr_A_base = 0x%16" PRIx64 "  ddr_A_lim = 0x%16" PRIx64 "\n",
	     ddr_A_base, ddr_A_lim);
    fprintf (stdout, "    ddr_B_base = 0x%16" PRIx64 "  ddr_B_lim = 0x%16" PRIx64 "\n",
	     ddr_B_base, ddr_B_lim);
    fprintf (stdout, "    out_of_bounds_addr                         = 0x%16" PRIx64 "\n",
	     out_of_bounds_addr);
#endif
    ...
    fprintf (stdout, "    ddr_B_base = 0x%16" PRIx64 "  ddr_B_lim = 0x%16" PRIx64 "\n",
	     ddr_B_base, ddr_B_lim);
    fprintf (stdout, "    ddr_C_base = 0x%16" PRIx64 "  ddr_C_lim = 0x%16" PRIx64 "\n",
	     ddr_C_base, ddr_C_lim);
    fprintf (stdout, "    ddr_D_base = 0x%16" PRIx64 "  ddr_D_lim = 0x%16" PRIx64 "\n",
    ...
    fprintf (stdout, "num_ERRORS = %0" PRId64 "\n", num_ERRORS);
    fprintf (stdout, "num_expected_ERRORS = %0" PRId64 "\n", num_expected_ERRORS);
    fprintf (stdout, "n_AXI4_reads  = %0" PRId64 ", AXI4_read_bytes  = %0" PRId64 "\n",
	     n_AXI4_reads, AXI4_read_bytes);

    fprintf (stdout, "n_AXI4_writes = %0" PRId64 ", AXI4_write_bytes = %0" PRId64 "\n",
	     n_AXI4_writes, AXI4_write_bytes);

    fprintf (stdout, "n_AXI4L_reads  = %0" PRId64 " (4 bytes each)\n", n_AXI4L_reads);
    fprintf (stdout, "n_AXI4L_writes = %0" PRId64 " (4 bytes each)\n", n_AXI4L_writes);
    fprintf (stdout, "----------------\n");

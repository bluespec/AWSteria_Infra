...
You should be able to see your all your PCIe devices with:

----
    $ lspci
----

By providing vendor and device ID, you can restrict it to the Xilinx VCU118:
...
your Linux host.  See instructions at:
...
_One-time step:_ load the fixed part of the bitfile from Garnet, which
contains the PCIe interface logic and DDR interface logic.  (I am not
sure where we got the `program_fpga` executable, but it loads a
bitfile into the FPGA.)

----
    $ program_fpga  $(GARNET_REPO)/shell/prebuilt/empty.bit
----

This step does not have to be repeated as we load new partial bitfiles
for applications.
...
We should have already loaded the fixed part of the bitfile from
Garnet (see Section Prerequisites above).

Loading the partial bitfile for TestApp uses the `xvsecctl` tool and
`xvsec` driver (see Section Prerequisites above).

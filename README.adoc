...
hardware side to run at slower clock speeds if needed.
...
----
Platform_Sim/    (Bluesim and Verilator sim)
Platform_AWSF1/
Platform_VCU118/
----
...
* The hardware side plugs into the so-called "`shell`" in the
    ...
    host-communication interfaces to the PCIe bus, and the DDR
    interfaces to DDRs on the FPGA board.
...
* The host side runs as a regular program on the Debian/Ubuntu host.
...
// ----------------
// SUBSUBSECTION
==== Prerequisites: Xilinx XDMA and XVSEC drivers
